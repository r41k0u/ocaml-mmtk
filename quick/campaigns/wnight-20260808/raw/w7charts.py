...
OUT = os.path.expanduser("~/shape/wcomp8")
...
# ---- fig7: D3 MMU (minimum mutator utilization) — the headline latency view ----
def mmu_curve(intervals, windows):
    """intervals: sorted (start, dur) pause list (s). MMU(w) = 1 - max STW
    in any w-window / w; the max is attained with a boundary at a pause
    start, so anchor the sweep there."""
    import bisect
    out = []
    n = len(intervals)
    starts = [s for s, _ in intervals]
    ends = [s + d for s, d in intervals]
    pre = [0.0]
    ...
    for w in windows:
        ...
        for i in range(n):
            ...
            stw = pre[j] - pre[i]
            if j - 1 >= i and ends[j - 1] > hi:
                stw -= ends[j - 1] - hi
            if stw > worst:
                worst = stw
        out.append(max(0.0, 1.0 - worst / w))
    return out

def fig7():
    order = ("binarytrees", "kb", "mature_mutation")
    windows = [0.001 * (10 ** (i / 8)) for i in range(0, 29)]
    ...
    for ax, b in zip(axes, order):
        for tag, color, label in ((f"v.{b}", S1, "vanilla"),
            ...
            recs = [r for r in ndjson(f"{OUT}/streams/{tag}.pause.ndjson") if "dur" in r]
            if not recs: continue
            iv = sorted((r.get("at", 0.0), r["dur"]) for r in recs)
            ys = mmu_curve(iv, windows)
            ax.plot([w * 1e3 for w in windows], ys, color=color, lw=2,
                    label=label, zorder=3)
        ax.set_xscale("log")
        ax.set_ylim(-0.02, 1.02)
        ax.set_xlabel("window size (ms, log)")
        ax.set_title(f"D3 — MMU: {SHORT[b]}")
        ax.grid(zorder=0)
        ax.legend(frameon=False, labelcolor=SEC, fontsize=8, loc="upper left")
    axes[0].set_ylabel("min mutator utilization")
    fig.tight_layout(); fig.savefig(f"{FIG}/fig7_d3_mmu.png", dpi=150); plt.close(fig)
