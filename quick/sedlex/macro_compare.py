#!/usr/bin/env python3
"""Macro benches: vanilla vs Bactrian (pre-gate, gate, v5 candidate).
Reads the panel logs under results/macro-gate/ and prints markdown tables;
writes charts/macro_vs_vanilla.png. Usage: macro_compare.py [results/macro-gate] [charts]"""
import re, sys, os, glob
R = sys.argv[1] if len(sys.argv) > 1 else 'results/macro-gate'; OUT = sys.argv[2] if len(sys.argv) > 2 else 'charts'
BENCH = [('test_decompress','decompress'),('eio_conc_bench','eio'),('liq_video_frames','liq video frames'),('sedlex_bench','sedlex 6M'),('ydump_repeat','ydump 6M'),('zarith_pi','zarith')]
def parse(path, side):
    d = {}
    if not os.path.exists(path): return d
    for l in open(path):
        m = re.match(rf"{side} (\S+): wall_med=([0-9.]+)s .*?rss=(\d+)MB (?:pauses=(\d+) fulls=(\d+) max_pause=(\d+)ms .*?GC time: (\d+) ms|minors=(\d+) majors=(\d+) promoted_words=\S+ gc_time=([0-9.?]+)s max_pause=([0-9.?]+)ms)", l)
        if not m: continue
    ...
    return d
van = parse(f'{R}/vanilla/panel.log', 'vanilla'); pre = parse(f'{R}/panel.log', 'gate-before'); gate = parse(f'{R}/panel.log', 'gate-after')
v5 = parse(f'{R}/rule1/panel-rule1v5.log', 'gate-rule1v5')
v5_filled = {}
for exe, _ in BENCH:
...
SERIES = [('vanilla', van), ('Bactrian pre-gate', pre), ('Bactrian gate (committed)', gate), ('Bactrian v5 (candidate)', v5_filled)]
def fmt(v, unit):
    if v is None: return '?'
...
table('maxp', 'ms', 'Max pause', 'Bactrian: longest STW window in the pause log (nursery, Full, sliced-cycle pause). Vanilla: `olly gc-stats` latency profile max (a minor plus its attached major slice).')
...
fig, axes = plt.subplots(2, 2, figsize=(12, 7.6), dpi=150); fig.patch.set_facecolor(SURFACE)
...
for ax,(key,lab,log) in zip(axes.flat, panels):
    ...
    if key=='maxp' and not any(van.get(exe,{}).get('maxp') for exe,_ in BENCH):
        ax.text(0.01, 0.97, 'vanilla: pending an idle machine (olly gc-stats)', transform=ax.transAxes, fontsize=8, color=TEXT2, va='top')
    ax.set_xticks(x); ax.set_xticklabels([n for _,n in BENCH], fontsize=8.5, color=TEXT1)
...
fig.suptitle('Macro benches, default rungs, dynamic heap, 1 GC worker: vanilla vs Bactrian (labels = v5 ÷ vanilla)', fontsize=11, color=TEXT1, x=0.01, ha='left')
fig.tight_layout(rect=(0,0,1,0.92)); os.makedirs(OUT, exist_ok=True); fig.savefig(f'{OUT}/macro_vs_vanilla.png', facecolor=SURFACE); print(f'\nchart: {OUT}/macro_vs_vanilla.png')
