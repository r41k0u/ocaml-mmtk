...
if sed or bt:
    ...
    for ax,(name,data) in zip(axes,[('sedlex n=1M — long-lived (nothing dies young)',sed),('binarytrees n=20 — short-lived control',bt)]):
        frame(ax); title(ax,name)
        for lbl,pts in data.items():
            c={'Bactrian (default)':ORANGE,'Bactrian, backstop off':MUTED,'Vanilla':AQUA}[lbl]
            xs=[p[0] for p in pts]; ys=[p[1] for p in pts]
            ax.plot(xs,ys,color=c,linewidth=2,marker='o',markersize=6,label=lbl)
            ax.text(xs[-1],ys[-1],f' {lbl}',fontsize=8.5,color=c,va='center')
        ax.set_xscale('log',base=2); ax.set_xlabel('nursery size, MB',fontsize=9.5,color=TEXT1); ax.set_ylabel('wall, s',fontsize=9.5,color=TEXT1)
        ax.set_ylim(bottom=0); ax.set_xlim(right=ax.get_xlim()[1]*3.5)
